...
export async function searchContacts(query: string): Promise<string[]> {
  ...
  try {
    ...
    directoryPeople.forEach((person: any) => {
    ...
    return storedAttendees.filter((email: string) => 
      email.toLowerCase().includes(query.toLowerCase())
    );
  }
}
...
async function checkEventConflicts(start: Date, end: Date, excludeEventId?: string): Promise<boolean> {
  if (!gapiInited || !gisInited) {
    throw new Error('Google Calendar API not initialized');
  }
...
export async function deleteEvent(eventId: string, deleteType: 'single' | 'future' | 'all'): Promise<void> {
  if (!gapiInited || !gisInited) {
    throw new Error('Google Calendar API not initialized');
  }
