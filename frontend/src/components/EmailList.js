...
const EmailList = ({ type }) => {
  ...
  useEffect(() => {
    ...
    console.log('EmailList effect triggered:', { type, search: location.search });
    fetchEmails();
  ...
  const fetchEmails = async () => {
    try {
      ...
      console.log('Fetching emails:', { type, searchQuery });
      
      const response = await axios.get(`http://localhost:5000/api/emails/${type}`, {
        params: { 
          email: userEmail,
          token: sessionToken,
          q: searchQuery
    ...
    if (selectedEmail && window.confirm('Are you sure you want to delete this message?')) {
      try {
        ...
        const response = await axios.delete(
          `http://localhost:5000/api/emails/${selectedEmail.id}`,
          {
            params: {
              email: userEmail,
              token: sessionToken,
